...
lrmodel_1 <- glm(yyes ~., data = group1_train, family = "binomial")
summary(lrmodel_1)
...
ANN_model1 <- neuralnet(formula = yyes ~ . - cluster, data = group1_train)
plot(ANN_model1)
```
...
ANN_pred1 <- predict(ANN_model1, group1_test)
threshold <- 0.5
predicted_yyes_1 <- ifelse(ANN_pred1 < threshold, 0, 1)
CrossTable(x = group1_test$yyes, y = predicted_yyes_1, prop.chisq=FALSE)
# confusionMatrix(as.factor(group1_test$yyes), as.factor(predicted_yyes_1), positive = "1")


# a more complex neural network topology with 5 hidden neurons
# ANN_model1 <- neuralnet(yyes ~ . - cluster, data = ANN_train, hidden = 5)
# plot(ANN_model1)
